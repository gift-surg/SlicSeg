function mex_files_to_compile_map = SlicSegGetMexFilesToCompile(~)
    ...
    files_to_compile{end + 1} = CoreMexInfo(1, 'wgtGLCM', 'cpp', fullfile(mex_dir, 'library', 'dwt'), [], ...
        {fullfile(mex_dir, 'library', 'dwt', 'pixelOperation.cpp'), fullfile(mex_dir, 'library', 'dwt', 'wgtCooccurence.cpp')} ...
    );
    
    % maxflow    
    files_to_compile{end + 1} = CoreMexInfo(1, 'maxflow_revisemex', 'cpp', fullfile(mex_dir, 'library', 'maxflow'), [], ...
        {fullfile(mex_dir, 'library', 'maxflow', 'maxflow-v3.0', 'graph.cpp'), fullfile(mex_dir, 'library', 'maxflow', 'maxflow-v3.0', 'maxflow.cpp')} ...
    );

    files_to_compile{end + 1} = CoreMexInfo(1, 'wgtmaxflowmex', 'cpp', fullfile(mex_dir, 'library', 'maxflow'), [], ...
        {fullfile(mex_dir, 'library', 'maxflow', 'maxflow-v3.0', 'graph.cpp'), fullfile(mex_dir, 'library', 'maxflow', 'maxflow-v3.0', 'maxflow.cpp')} ...
    );

    % Online random forest
    files_to_compile{end + 1} = CoreMexInfo(1, 'Forest_interface_mex', 'cpp', fullfile(mex_dir, 'library', 'OnlineRandomForest'), [], ...
        {fullfile(mex_dir, 'library', 'OnineRandomForest', 'ORForest.cpp'), fullfile(mex_dir, 'library', 'OnlineRandomForest', 'ODTree.cpp')} ...
    );
