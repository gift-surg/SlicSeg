...
public:
    ...
    myPrintf("Constructor[Block] should not be called\n");exit(-1);
    //first = last = NULL; block_size = size; error_function = err_function;
  }

	void initialize(int size, void (*err_function)(char *) = NULL) { first = last = NULL; block_size = size; error_function = err_function; }
  
	/* Destructor. Deallocates all items added so far */
	~Block() { 
    myPrintf("Destructor[Block] should not be called\n");exit(-1);
   //while (first) { block *next = first -> next; delete first; first = next; } 
   }
	...
	Type *New(int num = 1)
	{
		...
		if (!last || last->current + num > last->last)
		{
			...
			else
			{
				...
				block *next = (block *) myMalloc(sizeof(block) + (block_size-1)*sizeof(Type));
...
template <typename type> Block<type>* newBlock(int size, void (*err_function)(char *) = NULL){
  Block<type> *b=(Block<type>*)myMalloc(sizeof(Block<type>));
  b->initialize(size,err_function);
  return b;
}

template <typename type>
void deleteBlock(Block<type>** b){
  (*b)->cleanup();
  myFree(*b);
  *b=NULL;
}
...
public:
    ...
    myPrintf("Constructor[DBlock] should not be called\n");exit(-1);
    //first = NULL; first_free = NULL; block_size = size; error_function = err_function; 
  }

	void intialize(int size, void (*err_function)(char *) = NULL) { first = NULL; first_free = NULL; block_size = size; error_function = err_function; }

	/* Destructor. Deallocates all items added so far */
	~DBlock() {
    myPrintf("Destructor[DBlock] should not be called\n");exit(-1);
    //while (first) { block *next = first -> next; delete first; first = next; } 
  }
	...
	Type *New()
	{
		...
		if (!first_free)
		{
			...
			first = (block *) myMalloc(sizeof(block) + (block_size-1)*sizeof(block_item));
...
template <typename Type> DBlock<Type>* newDBlock(int size, void (*err_function)(char *) = NULL){
  DBlock<Type> *db=(DBlock<Type>*)myMalloc(sizeof(DBlock<Type>));
  db->intialize(size,err_function);
  return db;
}

template <typename Type>
void deleteDBlock(DBlock<Type>** db){
  (*db)->cleanup();
  myFree(*db);
  *db=NULL;
}
