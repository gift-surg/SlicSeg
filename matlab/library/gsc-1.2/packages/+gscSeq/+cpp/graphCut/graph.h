...
/*
	...
	the aforementioned paper in any resulting publication.
*/
/* -- Some implementation details added by Varun Gulshan ---
 * Graph is stored in adjacency list form. 
 * Each directed edge corresponds to a arc object.
 * Each node has a linked list of these arc objects (the arc objects themselves are nodes of this list
 * Also there is an array of pointers to these arc objects, which lets you access all the edges directly
 *  
 *  So space complextity: G=(V,E) with n nodes and m edges is: O(m+n)
 *  To be exact it is: m * (size of each arc object) + n * (size of each node object) + m*(size of pointer to each arc object)
 *  Memory might be a bit more, because here 'blocks.h' is being used for memory management i think, will read that later
 *  Size of arc object for Graph<int,int,int> = 20bytes
 *  Size of node object for Graph<int,int,int> = 44bytes
*/
...
public:
  ...
  // A construction like function, but used to initialize memory alloc with malloc.
	void initialize(int node_num_max, int edge_num_max, void (*err_function)(char *) = NULL);

  // clean up routine corresponding to the destructor
  void cleanup();
...
/// To initalize a graph for a mex file
template <typename captype, typename tcaptype, typename flowtype>
Graph<captype,tcaptype,flowtype>* newGraph(int node_num_max, int edge_num_max, void (*err_function)(char *)=NULL);
